...
def change(t_start, t_end, diff , idx, start, end):
    ...
    if start >= t_start and t_end >= end:
        ...
        if start != end:
            lazy[idx * 2] = diff ^ lazy[idx * 2]
            lazy[idx * 2 + 1] = diff ^ lazy[idx * 2 + 1]
        return
...
def update_lazy(idx, start, end):
    if (end - start + 1) % 2:
        tree[idx] = lazy[idx] ^ tree[idx]
    if start != end:
        lazy[idx * 2] = lazy[idx] ^ lazy[idx * 2]
        lazy[idx * 2 + 1] = lazy[idx] ^ lazy[idx * 2 + 1]
    lazy[idx] = 0
...
for _ in range(M):
    ...
    if order == 1:
        change(data[0] + 1, data[1] + 1, data[2], 1, 1, N)
    else:
        print(p_sum(data[0] + 1, data[1] + 1, 1, 1, N))